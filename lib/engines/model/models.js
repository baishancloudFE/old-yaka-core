...
const resolveParams = (params, auto, { formValueGettingFunction, getInitData, getState }) =>
  new Promise(resolve => {
    let _params = {}
    if (params) {
      setTimeout(() => {
...
const modelFactory = (model, yakaApis) => {
  ...
  const doFetch = async (auto = false) => {
    const _params = await resolveParams(params, auto, {
      formValueGettingFunction,
      getInitData,
      getState
    })
    ...
    if (type === 'get' || type === 'restful') {
      //支持url传递参数
      let newURL = changeURl(url, _params)
      fetch(newURL, {
        headers: {
          ...headers,
...
const resolveModels = (models, yakaApis) => {
  const _models = {}
